 ...
 * It's copied from https://github.com/hseeberger/akka-http-json/blob/master/akka-http-circe/src/main/scala/de/heikoseeberger/akkahttpcirce/CirceSupport.scala
 * We vendor it here so that we don't end up in a tangle of akka/akka-http dependencies
 */
...
object ErrorAccumulatingCirceSupport extends ErrorAccumulatingCirceSupport {
  final case class DecodingFailures(failures: NonEmptyList[DecodingFailure])
      extends Exception {
...
trait BaseCirceSupport {
  ...
  /**
    ...
    */
  implicit def fromByteStringUnmarshaller[A: Decoder]
  ...
  def byteStringJsonUnmarshaller: Unmarshaller[ByteString, Json] =
    Unmarshaller(
      _ => bs => Future.fromTry(jawn.parseByteBuffer(bs.asByteBuffer).toTry)
    )
  ...
  implicit def sourceUnmarshaller[A: Decoder](
    ...
    Unmarshaller
      .withMaterializer[HttpEntity, SourceOf[A]] {
...
trait FailFastUnmarshaller { this: BaseCirceSupport =>
  override implicit final def fromByteStringUnmarshaller[A: Decoder]
    : Unmarshaller[ByteString, A] =
    byteStringJsonUnmarshaller
      .map(Decoder[A].decodeJson)
      .map(_.fold(throw _, identity))

  override implicit final def unmarshaller[A: Decoder]
  ...
  override implicit final def fromByteStringUnmarshaller[A: Decoder]
    ...
    byteStringJsonUnmarshaller
      ...
      .map(
        _.fold(
          failures =>
            throw ErrorAccumulatingCirceSupport.DecodingFailures(failures),
          identity
        )
      )
