...
class AzureBlobMetadataTest
  ...
  def putObject(location: AzureBlobLocation,
                metadata: Map[String, String] = Map.empty): Unit = {
  ...
  override def withTags[R](
    ...
    testWith: TestWith[Tags[AzureBlobLocation], R]): R = {
    initialTags
      .foreach {
        case (location, tags) =>
          putObject(
  ...
  override def createIdent(context: Container): AzureBlobLocation =
    createAzureBlobLocationWith(context)
  ...
  it("throws an error if the tags are larger than 8KB in total") {
    withAzureContainer { container =>
      val location = createAzureBlobLocationWith(container)
      putObject(location)
