package weco.messaging.worker.monitoring.tracing
...
/**
  * Implements the [[weco.messaging.worker.steps.MonitoringProcessor]] interface with Opentracing (https://opentracing.io/).
  */
class OpenTracingMonitoringProcessor[Work, InfraServiceMonitoringContext](
  ...
  /**
    ...
    * If an optional `context` is passed, it uses a [[weco.messaging.worker.monitoring.tracing.OpenTracingSpanSerializer]]
