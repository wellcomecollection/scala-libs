...
def update_sbt_version(new_version_tag: str):
    with open(BUILD_SBT, 'r') as f:
        lines = f.readlines()

    new_version = new_version_tag.strip('v')
    
    for idx, line in enumerate(lines):
        if line.startswith('val projectVersion = '):
            lines[idx] = f'val projectVersion = "{new_version}"\n'
            break
    else:  # no break
...
def create_release():
    ...
    update_sbt_version(new_version_tag)
